...
func printUsage() {
	...
	fmt.Println("  --semver                Publish semver tag (requires clean git)")
	fmt.Println("  --multi-tag             Publish latest/major/minor/full tags (requires --semver --push)")
	fmt.Println("  --label key=value       OCI label for podman build (repeatable)")
	...
	fmt.Println("  CHART_REGISTRY                  Chart registry base path (without oci://)")
	fmt.Println("  CHART_REGISTRY_USERNAME         Chart registry username")
	fmt.Println("  CHART_REGISTRY_PASSWORD         Chart registry password")
